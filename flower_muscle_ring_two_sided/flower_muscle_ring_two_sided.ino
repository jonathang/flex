...
void setup() {
  ...
  if (DEBUG || DEBUGANALOG) Serial.begin(9600); 
  strip.begin();
...
void muscleTwoSides() {
  ...
  if (DEBUG || DEBUGANALOG) Serial.println("Current sensor value is: " + String(sensorValue));
  float actualPercent = (float)sensorValue / 4;
  if (DEBUG) Serial.println(actualPercent);
  runLeftSide(int(actualPercent + 0.5));
  runRightSide(int(actualPercent + 0.5));
  strip.show();
}
...
//engage pixels 0-7
void runLeftSide(int percent) {
  if (DEBUG) Serial.println("got percent: " + String(percent));
  int lastLitPin = (8.00 * (float)percent / 100.00) +  0.5;
  if (DEBUG) Serial.println("last lit pixel: " + String(lastLitPin));
  for (int  i = 0; i < 4 ; i++) {
    if (i < lastLitPin) {
      strip.setPixelColor(i, 255, 0, 0);
      if (DEBUG) Serial.println("marking pixel #" + String(i) + " with the color red");
    } else {
      strip.setPixelColor(i, 0, 0, 0);
      if (DEBUG) Serial.println("marking pixel #" + String(i) + " off");
    }
  }
  ...
  for (int j = 4; j < 8 ; j++) {
    if (j < lastLitPin) {
      ...
      if (DEBUG) Serial.println("marking pixel #" + String(j) + " with the color green");
    } else {
      strip.setPixelColor(j, 0, 0, 0);
      if (DEBUG) Serial.println("marking pixel #" + String(j) + " off");
    }   
  }
}

//engage pixels 16-8
void runRightSide(int percent) {
  if (DEBUG) Serial.println("got percent: " + String(percent));
  int lastLitPin = 16.00 - ((8.00 * (float)percent / 100.00) +  0.5);
  if (DEBUG) Serial.println("last lit pixel: " + String(lastLitPin));
  for (int  i = 15; i > 11 ; i--) {
    if (i > lastLitPin) {
      strip.setPixelColor(i, 255, 0, 0);
